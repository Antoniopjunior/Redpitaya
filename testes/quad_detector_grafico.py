...
ax.setylabel("Y Position (normalized)")
...
# Barra de intensidade SUM (como no Kinesis)
...
MAX_HISTORY = 50  # Número máximo de pontos no histórico
...
try:
    ...
    while True:
        ...
        if all(v is not None for v in [sum_voltage, x_voltage, y_voltage]):
            ...
            # Atualiza barra de SUM

            ax.patches.clear()  # Remove barras anteriores

            sum_bar_value = max(0, min(1.0, sum_voltage / 10.0))  # Normaliza para 0-10V

            ax.barh(-1.3, sum_bar_value * 2.4, height=0.1, color='blue', alpha=0.7)
            ...
            print(f"SUM: {sum_voltage:.3f}V, X: {x_voltage:.3f}V, Y: {y_voltage:.3f}V")
