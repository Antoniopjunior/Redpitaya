...
while time.time() - start_time < tempo_total_segundos:
    ...
    if now >= next_acquisition:
        ...
        max1, max2, max3, max4 = np.max(buff1), np.max(buff2), np.max(buff3), np.max(buff4)
        ...
        print(f"Amplitudes máximas - CH1: {max1:.2f}V, CH2: {max2:.2f}V, CH3: {max3:.2f}V, CH4: {max4:.2f}V")
...
for i, leitura in enumerate(todas_as_leituras):
    ...
    plt.semilogy(freq/1e6, fft1, color=cores[0], alpha=0.7, label=f"CH1 (aq.{i+1})" if i == 0 else "")
    plt.semilogy(freq/1e6, fft2, color=cores[1], alpha=0.7, label=f"CH2 (aq.{i+1})" if i == 0 else "")
    plt.semilogy(freq/1e6, fft3, color=cores[2], alpha=0.7, label=f"CH3 (aq.{i+1})" if i == 0 else "")
    plt.semilogy(freq/1e6, fft4, color=cores[3], alpha=0.7, label=f"CH4 (aq.{i+1})" if i == 0 else "")
