...
IP = '10.42.0.25' #SO Windows: 169.254.56.223 | SO: Linux Ubuntu 10.42.0.25
rp_s = scpi.scpi(IP)
...
global_max = np.max(amplitudes_maximas[1:])
global_min = min(min(amplitudes_maximas[1:]))
margin = 0.1
# Exemplo: escala Y fixa de 0V a 5V
plt.ylim(global_min - margin, global_max+margin)
print(amplitudes_maximas[2:])
...
max1 = [x[1] for x in amplitudes_maximas]
max2 = [x[2] for x in amplitudes_maximas]
max3 = [x[3] for x in amplitudes_maximas]
max4 = [x[4] for x in amplitudes_maximas]
